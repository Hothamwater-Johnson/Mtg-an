  ...
  return (
    <form action={formAction} className="space-y-6">
              ...
              <div key={row.id} className={cn('transition-colors', !row.is_included && 'opacity-50 bg-gray-50')}>
                ...
                <div className="flex items-start gap-3 px-4 py-3">
                  ...
                  <button
                    ...
                    title={row.is_included ? 'Exclude from proposal' : 'Include in proposal'}
                  >
                    {row.is_included
                  ...
                  {/* Category badge */}
                  <span className={cn(
                    'shrink-0 mt-0.5 inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-medium uppercase tracking-wide',
                  ...
                  <input
                    ...
                    className="flex-1 min-w-0 text-sm text-gray-900 bg-transparent border-0 focus:outline-none focus:ring-0 truncate"
                  />
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <input
                      ...
                      className="w-16 text-xs text-right bg-transparent border-0 focus:outline-none focus:ring-0 text-gray-600"
                    />
                ...
                {/* Expanded detail */}
                {row.expanded && (
                  <div className="px-12 pb-4 space-y-3 bg-gray-50 border-t border-gray-100">
                    {/* Cost fields */}
                    <div className="grid grid-cols-3 gap-3 text-xs">
                      <label className="space-y-1">
                        <span className="text-gray-500">Unit material ($)</span>
                        <input
                      ...
                      <label className="space-y-1">
                        <span className="text-gray-500">Unit labor ($)</span>
                        <input
                          type="number" min={0} step={0.01}
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        <div className="space-y-2 text-sm max-w-xs ml-auto">
          <div className="flex justify-between text-gray-600">
            <span>Materials</span>
            <span>{formatCurrency(totals.totalMaterial)}</span>
          </div>
          <div className="flex justify-between text-gray-600">
            <span>Labor</span>
      ...
      <div className="flex justify-end gap-3">
        ...
        <Button type="submit" name="_action" value="continue" disabled={pending}>
          Continue to quote →
        </Button>
      </div>
    </form>
